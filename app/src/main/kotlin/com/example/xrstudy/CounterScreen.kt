...
fun CounterScreen(modifier: Modifier = Modifier) {
    ...
    Column(
        ...
        CounterCard(
            ...
            count = rememberCount,
            // remember には increment() のような関数がないので、
            // 状態を書き換えている onClick 側にログを書く。
            // （Composable の本体に書くと、recomposition のたびに出てしまう）
            onIncrement = {
                rememberCount++
                Log.d("LIFECYCLE", "remember.increment  ← count=$rememberCount")
            },
            onReset = {
                rememberCount = 0
                Log.d("LIFECYCLE", "remember.reset  ← count がリセット")
            }
        )
        ...
        CounterCard(
            ...
            onIncrement = {
                saveableCount++
                Log.d("LIFECYCLE", "rememberSaveable.increment  ← count=$saveableCount")
            },
            onReset = {
                saveableCount = 0
                Log.d("LIFECYCLE", "rememberSaveable.reset  ← count がリセット")
            }
        )
        ...
        Column(
            ...
            Row(
                ...
                Text(
                    ...
                    // 状態を見て if で分岐するだけ。count が変わると再描画され、色が変わる。
                    // Color.Unspecified は「色の指定なし」（テーマ本来の文字色になる）。
                    color = if (count == 3) MaterialTheme.colorScheme.error else Color.Unspecified,
